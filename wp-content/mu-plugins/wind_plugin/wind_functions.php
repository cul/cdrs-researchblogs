 ...
 * this includes the main blog and the current blog, if blog privacy >= -1
...
function wind_add_to_blogs ($user, $wind_affiliations, $debug) {
	...
	$user_current_blogs = get_blogs_of_user($user->ID);
	$needs_subscribe_to_main_blog = TRUE;
	$needs_subscribe_to_current_blog = FALSE;
	$not_subscribing_current_reason =" (blog is private)";
	
	// need to check if current blog is public (wp_blogs.public >= -1, "visible only to registered users of whole site")
	$current_blog_public = $current_blog->public;
	if ($current_blog_id !=1 and $current_blog_public > -2) {
		$needs_subscribe_to_current_blog = TRUE;
	} 
		...
		$affilCourse = $this_affil->_wpmuCourseNumber;
			...
			foreach ($all_blogs as $wpmu_blog) {
				...
				if ( $blog_path == $affilCourse ) {
					// we found a matching blog for the affil
					$this_affil->set_subscribed(true);
					$this_affil->set_blog_id($blog_id);
					
					// does the user need to be added?
					if ( wind_user_in_blog($user->ID, $blog_id) ) {
						// nope, they're already added
						windlogger("{$this_affil->_uni} already subscribed to {$this_affil->_wpmuCourseNumber}");
						
					} else {
						// yep, subscribe them
						windlogger("Subscribing $user->ID $user->user_login to $blog_path, $blog_id as " . get_blog_role_option($wpmu_blog->blog_id, $this_affil->_role ) );
					...
					} // end if wind_user_in_blogs
					
					// stop checking additional blogs, since we found a match
					break;
				} // end if blog path == affilCourse
			} // end foreach "all blogs"
			...
			} // end if is null this affil is subscribed
			
	} // end foreach on affiliations array
	
	// subscribe user to main blog if they're not already
	if ($needs_subscribe_to_main_blog && !wind_user_in_blog($user->ID, 1)) {
		windlogger("Subscribing $user->ID $user->user_login to main blog");
		add_user_to_blog(1, $user->ID, "subscriber");
	}

	// subscribe to the current blog if it's public-ish and they're not already subscribed
	if ($needs_subscribe_to_current_blog && !wind_user_in_blog($user->ID, $current_blog_id)) {
		windlogger("Subscribing $user->ID $user->user_login to current blog $current_blog_id privacy $current_blog_public");
		add_user_to_blog($current_blog_id, $user->ID, "subscriber");
	} else {
		windlogger("NOT subscribing $user->ID $user->user_login to current blog $current_blog_id privacy $current_blog_public $not_subscribing_current_reason");
	}
...
function wind_create_wp_user( $user_name, $wind_affiliations ) {
	...
	if ($wind_check_course_affils) {
							error_log("yes check course affils for {$user_id}\n", 3, $wind_log_file);
...
function wind_authenticate () {
	...
	if($_GET["ticketid"]) {
	  ...
	  $USE_WGET = false;
	    ...
	    if ($USE_WGET) {
	      ...
	      $wget_args = " --ca-certificate $WIND_WP_PATH/wp-content/mu-plugins/wind_plugin/intermediateCAbundle.crt -q -O - $validate";
	      $output = wrap_wget($wget_args);
	      list($firstline, $secondline) = wrap_wget($wget_args);
	      $wind_response = $output[0];
	      $UNI = $output[1];
	      windlogger("$UNI logging in");
	    } else {
	      // using php curl instead
	      $curli = curl_init();
	      curl_setopt($curli, CURLOPT_URL, $validate);
	      curl_setopt($curli, CURLOPT_SSL_VERIFYPEER, true);
	      curl_setopt ($curli, CURLOPT_RETURNTRANSFER, 1);
	      curl_setopt($curli, CURLOPT_CAINFO, "$WIND_WP_PATH/wp-content/mu-plugins/wind_plugin/intermediateCAbundle.crt");
	      
	      $raw_curl_response = trim(curl_exec($curli));
	      $cerror = curl_error($curli);
	      //windlogger("wind response: " . print_r($raw_curl_response, true));

	      $wind_auth_status = 0; //by default

	      if($raw_curl_response[0] == '<') {
		//windlogger("Parsing xml...");

		//We're dealing with XML
		$xml = new SimpleXMLElement($raw_curl_response);

                //Get namespace and remove it from the child elements so that we can work with the SimpleXMLElement more easily.
                $namespaces = $xml->getNameSpaces(true);
                $deNamespacedXml = $xml->children($namespaces['wind']); //'http://www.columbia.edu/acis/rad/authmethods/wind'

                $user_uni = (string)$deNamespacedXml->authenticationSuccess->user;
                
		$user_affiliations = array();

                foreach($deNamespacedXml->authenticationSuccess->affiliations->affil as $affiliation) {
                  $user_affiliations[] = (string)$affiliation;
                }

                $UNI = $user_uni;
                $wind_affiliations = $user_affiliations;
		...
		//We're dealing with a plaintext response
		$response_lines = explode("\n", $raw_curl_response);
		$wind_auth_status = ($response_lines[0] == 'yes' ? 1 : 0);
		$UNI = $response_lines[1];
		$wind_affiliations = array_slice($raw_curl_response, 2, (sizeof($raw_curl_response)-2), false);

		//windlogger("first line is $response_lines[0] - error $cerror");
		//windlogger("$UNI logging in");
	      }
	...
	//windlogger('auth_status: ' . print_r($wind_auth_status, true));
	...
	if ( $wind_auth_status ) {
	  ...
	  if ( $user = get_userdatabylogin($UNI) ) { // user already exists
	    if ($debug) { windlogger("user $UNI already exists ID $user->ID ");}
	  ...
	  windlogger("login link is $login_link");
...
// function to send user to correct login url
function wind_login_redirect() {
	global $current_blog;
	
	if ($current_blog->blog_id == 1) {
		#windlogger("current site id is $current_blog->blog_id , redirecting to my sites");
		wp_redirect( site_url( '/wp-admin/my-sites.php' ));
	} else {
		#windlogger("current site id is $current_blog->blog_id , redirecting to front end");
		wp_redirect( site_url( '/' ));
	}
	
	
} //end function wind_login_redirect()
...
function wind_logout() {
	...
	if ($_SERVER["SERVER_PORT"] == 443) {
  	...
  	$destination = $server_protocol . "://" . $_SERVER["SERVER_NAME"]; // . $_SERVER["PHP_SELF"]; //redirect to blogs.cul home page
	$logout_link = "https://" . $wind_server . $wind_logout_uri . "?service=" . urlencode($destination);
...
/**
 * determines whether given user is already member of given blog
 * added 2011/10/12 nco2104
 * @param $userID is an integer
 * @param $blogID is an integer
 * @return true if user is member of blog, otherwise false
 */
function wind_user_in_blog($userID, $blogID) {
	global $wpdb;
	
	$thisUser = get_user_by('id', $userID);
	$cap_key = $wpdb->base_prefix . $blogID . '_capabilities';
	
	  if ( is_array($thisUser->$cap_key) && in_array(1, $thisUser->$cap_key) )
           return true;
   
       return false;	
} // end function wind_user_in_blog($userID, $blogID)
