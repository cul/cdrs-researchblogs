...
function wind_addProvisionUsersPage() {
		...
		add_submenu_page( 'users.php', 'Provision Users', 'Provision Users', 'manage_network_options', 'wind_provision_users', 'wind_provisionUsersPage');
		} 
...
function wind_addSettingsPage() {
		if (function_exists('add_submenu_page')) {        
			...
			add_submenu_page( 'settings.php', 'WIND Login Settings', 'WIND Login Settings', 'manage_network_options', 'wind_wind_settings', 'wind_settingsPage');
		} 
...
add_action('admin_menu', 'wind_addBlogUsersPage');

function wind_addBlogUsersPage() {
		if (function_exists('add_submenu_page')) {        
			add_submenu_page( 'users.php', 'Add by UNI', 'Add by UNI', 'add_users', 'wind_blog_users', 'wind_blogUsersPage');
		} 
} // end wind_addBlogUsersPage()

function wind_blogUsersPage() {
	global $wpdb, $wp_rewrite, $blog_id;
	$wind_wp_path =  explode("wp-content/", dirname(__FILE__)); 
	...
	?>
	<div class="wrap"> 
		<h2>Add Users to Your Blog by UNI</h2>
	<p>Enter each uni you want to add to your blog on a separate line below. 
		<br> - If the user doesn't have an account, one will be created for them.</li>
		<br> - If the user already has an account, their account will be updated with their new role on your blog.</li>
		 
		</p>
	<form method="post" id="wind_new_users">
	
	<?php 
	echo "<input name='wind_new_blog' id='wind_new_blog' type='hidden' value='$blog_id'>"; 
	?>
	
		<?php wp_nonce_field('wind_new_users') ?>
		<fieldset class="options">
                <table class="form-table" cellpadding="3" cellspacing="3">
 				<th scope="row"><label for="wind_new_roles"><?php _e('Grant this role:') ?></label></th>
				<td><?php wind_roles_dropdown() ?></td>
			</tr>
			<tr valign="top">
                <th scope='row'><label for="wind_new_users"><?php _e('to ALL of the following UNIs:') ?><br><br><br> 
                	<i>Enter each uni on a separate line.<br><Br> Example:<br>uni1<br>uni2<br>uni3</i></label></th>
                <td><textarea name="wind_new_users" id="wind_new_users" rows="15" cols="40"></textarea></td>
            </tr>
			<tr valign="top">
                </table>
                <p class="submit">
                        <input class="button" type="submit" name="windProvisionUsers" value="<?php _e('Add/Update These Users') ?>" />
		</p>
		</fieldset>
	</form>
		...
		<?php
} // end wind_blogUsersPage()

############# DE-PROVISION USERS PAGE #################
function wind_addDerovisionUsersPage() {
		if (function_exists('add_submenu_page')) {        
			add_submenu_page( 'users.php', 'DE-provision Users', 'DE-provision Users', 'manage_network_options', 'wind_deprovision_users', 'wind_deprovisionUsersPage');
		} 
} // end wind_addDeprovisionUsersPage()

function wind_deprovisionUsersPage() {
	global $wpdb, $wp_rewrite;
	$wind_wp_path =  explode("wp-content/", dirname(__FILE__)); 
	...
	# process updates on post
	if ($_SERVER['REQUEST_METHOD'] == 'POST') wind_deprovisionUsers();
	...
	if (isset($_GET['updated'])) {
	?>
	<div id="message" class="updated"><p><?php _e( 'Deprovisioning the user...' ) ?></p></div>
	<?php
		}
	?>
	<div class="wrap"> 
		<h2>De-provisioning WIND Users... </h2>
	<p> This script does several things:
		<ol>
			<li> Unsubscribes the user from all blogs.</li>
			<li> Checks all blogs admin email, and changes if it matches the user email.</li>
			<li> Removes the user's email from subscribe2 in all blogs.</li>
		</ol>
		
	</p>
	<form method="post" id="wind_deprovision_users">
		<?php wp_nonce_field('wind_deprovision_users') ?>
		<fieldset class="options">
			
        <table class="form-table" cellpadding="3" cellspacing="3">
            <tr valign="top">
                    <th scope='row'><label for="uni_to_remove"><?php _e('UNI to remove:') ?></label></th>
                    <td><input name="uni_to_remove" type="text" id="uni_to_remove" class="regular-text" 
                    	value="xyz123" /></td>
            </tr>
			<tr valign="top">
 				<th scope="row"><label for="replacement_email"><?php _e('Replacement email, if user is administrator:') ?></label></th>
				<td><input name="replacement_email" type="text" id="replacement_email" class="regular-text" 
                    	value="sysadmin@libraries.cul.columbia.edu" /><br>
                    	(This email address will be used as "admin email" for any blog where the user's email is currently listed as the "admin email".)</td>
        </table>
        
                <p class="submit">
                        <input class="button" type="submit" name="wind_Deprovision_Users" value="<?php _e('De-provision user') ?>" />
		</p>
		</fieldset>
	</form>
		...
		<?php
} // end wind_deprovisionUsersPage()

function wind_deprovisionUsers() {
	global $wpdb;
	require_once("wind_functions.php");
	if($_POST['uni_to_remove'] && $_POST['replacement_email']) {
		print "<div id='message' class='updated fade'><p>Deprovisioning user...</p>";
		$uni = $_POST['uni_to_remove'];
		$replacement_email = $_POST['replacement_email'];
		$all_blogs = $wpdb->get_results( "SELECT blog_id, path from wp_blogs" );
		
		if ( $user = get_userdatabylogin($uni)) {
			$user_id = $user->ID;
			
			print "<b>Revoking super admin privileges, if any...</b><br>";
			revoke_super_admin( $user_id );
			
			print "<b>Removing all subscribe2 subscriptions, if any...</b><br>";
			$sql = "delete from wp_usermeta where user_id =$user_id and meta_key like '%s2%' ";
			$rows = $wpdb->query($sql);
			if ($rows) print "$rows DB rows deleted.<br>";
			
			print "<br><b>Checking each blog for membership, admin email, and additional subscribe2 subscriptions</b> <br>";
			foreach ($all_blogs as $wpmu_blog) {
					$blog_path = $wpmu_blog->path;
					$blog_id = $wpmu_blog->blog_id;
					$blog_url = site_url( $blog_path . "wp-admin/");
					$admin_email = get_blog_option($blog_id, "admin_email");
					
					print "<br>Checking <a href='$blog_url'>$blog_path</a>...<br>";
					
					#check for membership
					if (is_user_member_of_blog( $user_id, $blog_id )) {
						print "&nbsp; &nbsp; &nbsp; <b>Removing</b> $uni from $blog_path...<br>";
						remove_user_from_blog($user_id, $blog_id);
						$no_matches = FALSE;
					} # end check for membership
					
					
					# check for admin mail match
					# need to match both uni@columbia.edu and uni+whatever@columbia.edu
					$pattern = "/^${uni}(\+[a-z0-9\-_\.]*)?@columbia\.edu\$/";
					if (preg_match($pattern, $admin_email)){
						print "&nbsp; &nbsp; &nbsp; <b>Changing</b> $admin_email on $blog_path... ";
						update_blog_option($blog_id, "admin_email", $replacement_email);
						$new_admin_email = get_blog_option($blog_id, "admin_email");
						print "changed to $new_admin_email<br>";
					} else {
						print "&nbsp; &nbsp; &nbsp; $admin_email is admin for $blog_path, keeping <br>";
					} # end check for admin mail match

				# remove from subscribe2
				$sql = "delete from wp_${blog_id}_subscribe2 where email = '${uni}@columbia.edu' or email like '${uni}+%@columbia.edu'";
				$rows = $wpdb->query($sql);
				if ($rows) print "Found additional subscribe2 matches... $rows DB rows deleted.<br>";
					
				} # end foreach over all blogs
			
			
			
			} else { # user does not exist
				print "The user $uni does not exist here. Double-check the username?";				
			} # end checking if user is valid	

		print "</div>";
		
    }	else {
    	// some field was missing 
    	print "<div id='message' class='updated fade'><p>All fields are required, even replacement email.</p>";
		print "</div>";
    }
} // end wind_deprovisionUsers()
